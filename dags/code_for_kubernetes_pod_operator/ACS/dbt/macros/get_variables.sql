{% macro get_variables(variables, select_col, year) %}
    ...
    {%- set prev_seq = first_seq -%}
        ...
        from ({{ get_variables_from_seq( variables, first_seq, year ) }}) as e{{first_seq}}
    {% if seq_list|length > 1 %}
        {% for seq in seq_list[1:] %}
            full outer join ({{ get_variables_from_seq( variables, seq, year ) }}) as e{{seq}}
